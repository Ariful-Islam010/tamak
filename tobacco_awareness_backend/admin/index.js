...
router.get('/api/stats', requireAdmin, async (req, res) => {
  try {
    const [users, usersToday, messages, goals, checkins, sos, storage, activeUsers7d, newUsers7d,
      ...
      query("SELECT COUNT(DISTINCT user_id) FROM daily_checkins WHERE used_tobacco = false"),
    ]);
    res.json({
...
router.get('/api/users/:id', requireAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    const [profile, goals, checkins, savings, gamification, sos, messages, files] = await Promise.all([
      ...
      query('SELECT * FROM savings_logs WHERE user_id = $1 ORDER BY created_at DESC', [id]),
    ...
    const smokeFreeDays = checkins.rows.filter(c => !c.used_tobacco).length;
...
router.get('/api/users/:id/export/csv', requireAdmin, async (req, res) => {
  try {
    ...
    const sections = {
      ...
      daily_checkins: await query('SELECT id, check_in_date, mood, craving_level, used_tobacco, note, created_at FROM daily_checkins WHERE user_id = $1 ORDER BY check_in_date DESC', [id]),
      ...
      savings_logs: await query('SELECT id, amount, created_at FROM savings_logs WHERE user_id = $1 ORDER BY created_at DESC', [id]),
      gamification: await query('SELECT current_streak, longest_streak, badges, last_check_in_date, updated_at FROM gamification_progress WHERE user_id = $1', [id]),
  ...
  try {
    const result = await query(`
      SELECT c.id, u.email, up.display_name, c.check_in_date, c.mood, c.craving_level,
        c.used_tobacco, c.note, c.created_at
      FROM daily_checkins c
      LEFT JOIN user_profiles up ON c.user_id = up.id
...
router.get('/api/savings/export/csv', requireAdmin, async (req, res) => {
  try {
    const result = await query(`
      SELECT s.id, u.email, up.display_name, s.amount, s.created_at
      FROM savings_logs s
      LEFT JOIN user_profiles up ON s.user_id = up.id
      LEFT JOIN users u ON s.user_id = u.id ORDER BY s.created_at DESC`);
    res.setHeader('Content-Type', 'text/csv');
...
router.get('/api/gamification', requireAdmin, async (req, res) => {
  try {
    const result = await query(`
      ...
      LEFT JOIN user_profiles up ON g.user_id = up.id
      LEFT JOIN users u ON g.user_id = u.id
      ORDER BY g.longest_streak DESC`);
...
router.get('/api/gamification/export/csv', requireAdmin, async (req, res) => {
  try {
    const result = await query(`
      SELECT u.email, up.display_name, g.current_streak, g.longest_streak, g.badges, g.last_check_in_date, g.updated_at
      FROM gamification_progress g
      LEFT JOIN user_profiles up ON g.user_id = up.id
      LEFT JOIN users u ON g.user_id = u.id ORDER BY g.longest_streak DESC`);
    res.setHeader('Content-Type', 'text/csv');
...
router.get('/api/analytics', requireAdmin, async (req, res) => {
  try {
    const [
      ...
      query(`SELECT DATE(check_in_date) as day, COUNT(*) as smoke_free FROM daily_checkins WHERE used_tobacco = false AND check_in_date >= NOW() - INTERVAL '30 days' GROUP BY day ORDER BY day`),
      ...
      query(`SELECT u.email, up.display_name, g.longest_streak, g.current_streak, g.badges FROM gamification_progress g LEFT JOIN user_profiles up ON g.user_id = up.id LEFT JOIN users u ON g.user_id = u.id ORDER BY g.longest_streak DESC LIMIT 10`),
    ]);
    res.json({
...
router.get('/api/export-all/csv', requireAdmin, async (req, res) => {
  try {
    const tables = {
      ...
      daily_checkins: `SELECT c.id, u.email, up.display_name, c.check_in_date, c.mood, c.craving_level, c.used_tobacco, c.note, c.created_at FROM daily_checkins c LEFT JOIN user_profiles up ON c.user_id = up.id LEFT JOIN users u ON c.user_id = u.id ORDER BY c.check_in_date DESC`,
      money_saver_goals: `SELECT g.id, u.email, up.display_name, g.title, g.target_amount, g.current_amount, g.is_completed, g.icon_name, g.created_at FROM money_saver_goals g LEFT JOIN user_profiles up ON g.user_id = up.id LEFT JOIN users u ON g.user_id = u.id`,
      savings_logs: `SELECT s.id, u.email, up.display_name, s.amount, s.created_at FROM savings_logs s LEFT JOIN user_profiles up ON s.user_id = up.id LEFT JOIN users u ON s.user_id = u.id`,
      gamification_progress: `SELECT u.email, up.display_name, g.current_streak, g.longest_streak, g.badges, g.last_check_in_date, g.updated_at FROM gamification_progress g LEFT JOIN user_profiles up ON g.user_id = up.id LEFT JOIN users u ON g.user_id = u.id ORDER BY g.longest_streak DESC`,
